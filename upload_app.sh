...
# and the first item listed is python. the next item is a dev dependency, so we
# can also skip it. we can then skip the first 6 lines to extract the names of 
# the packages, then we can use conda list to find out exactly what version of 
# the packages we are using
awk '(NR>6) { gsub(/^\s+-\s+|=.*$/, """"); print $0 }' environment.yml > /tmp/.requirements &&
  conda list --name=nhp |
  awk '{print $1 "==" $2}' |
